...
export default function InventoryEditor() {
    ...
    const [showSlotDetectionSettings, setShowSlotDetectionSettings] =
        useState(false);
        ...
        if (editingSlot !== null) {
            setItems((prevItems) => {
                const newItems = prevItems.filter(
                    (item) => item.position !== editingSlot,
                );
                return [
                    ...newItems,
                    {
    ...
    const handleDrop = (position: number) => {
        if (draggedItem) {
            const draggedItemObj = items.find(
